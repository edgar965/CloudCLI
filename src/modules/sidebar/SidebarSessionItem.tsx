...
import { Check, Copy, Edit2, GitBranch, Loader2, MoreHorizontal, Square, Trash2, X } from 'lucide-react';
...
import { useSelectionContext } from '@/modules/sidebar/hooks/SessionSelectionContext';
...
function SidebarSessionItem({
   ...
   * conversations, and the mark is easy to miss entirely - the first attempt
   * at this opened chats instead, which looks exactly like "only the last one
   * gets selected".
   */
   ...
   * top of that list, and finding the chat one is actually in meant scrolling
   * for it. `nearest` means a row already on screen is left where it is, so
   * switching sessions by clicking does not jerk the list around.
   */
  useEffect(() => {
    if (!isSelected) {
      return;
      ...
      <div>
        <a
          ...
          className={cn(
            ...
            // The open session carries the strongest mark in the list: a bar
            // in the accent colour and a tinted background. At 5% it was all
            // but invisible next to the solid "New Session" button, which made
            // the button look like the active chat.
            isSelected
              ? 'relative border-primary/30 bg-primary/15 before:absolute before:left-0 before:top-1/2 before:h-6 before:w-[3px] before:-translate-y-1/2 before:rounded-r-full before:bg-primary'
              : 'border-border/30',
            !isSelected && isProcessing
          ...
          // Ctrl/Cmd-click picks a row, shift-click the range up to it - the
          // gesture every list uses. It has to preventDefault: the row is an
          // <a>, and the browser's own meaning for those two keys is "open the
          // href in a new tab", which lands on the login screen. A new tab is
          // still reachable by middle-click and the right-click menu.
          // Alt-click stays the browser's (download).
          onClick={(event) => {
        ...
        >
          <div className="flex w-full min-w-0 items-center gap-2">
            {/* The provider mark doubles as the checkbox: hovering the row, or
