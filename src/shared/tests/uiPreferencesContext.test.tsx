...
test('a toggle persists to the single stored blob', () => {
  ...
  // Switched off, not on: dictation now defaults to on, and setting a
  // preference to the value it already has is not a change - the reducer
  // returns the same state and nothing is written. The test is about the
  // write, so it has to make a real change.
  act(() => {
    result.current('voiceEnabled', false);
  });
