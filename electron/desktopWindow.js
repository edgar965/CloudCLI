...
export class DesktopWindowManager {
  ...
  async showTarget(target, { trackTab = true } = {}) {
    ...
    // target.url, nicht finalUrl: showContentTarget gibt nichts zurueck, die
    // Bedingung wäre also immer falsch und der Token nie gesetzt worden.
    if (target.kind === 'local' && target.url) {
      void this.viewHost.syncSharedUiTokenForOrigin(target.url);
    }
